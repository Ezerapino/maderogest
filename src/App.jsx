...
function generarPDF(obra) {
  ...
  const mueblesHTML = (obra.muebles || []).map((m, i) => `
    <div class="mueble-row">
      <div class="mueble-num">${String(i+1).padStart(2,"0")}</div>
      <div class="mueble-icon">${muebleIcon(m)}</div>
      <div class="mueble-nombre">${m}</div>
...
<div class="cuerpo">
  ...
  <div class="seccion-titulo">🪵 Lista de muebles a entregar (${(obra.muebles||[]).length} ítems)</div>
  ${(obra.muebles||[]).length === 0 ? '<p style="color:#8a7860;font-size:13px">Sin muebles registrados.</p>' : mueblesHTML}
...
function ModalObra({ obra, onClose, onSave }) {
  const [form, setForm] = useState({
    ...
    muebles: obra?.muebles ? [...obra.muebles] : [],
    notas: obra?.notas || "",
  });
  ...
  function addMueble() {
    ...
    if (!v) return;
    setForm(f => ({ ...f, muebles: [...f.muebles, v] }));
    setMuebleInput("");
  }
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", backdropFilter:"blur(8px)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#13100a", border:"1px solid rgba(200,147,58,0.2)", borderRadius:20, padding:"28px 24px", width:"100%", maxWidth:520, maxHeight:"90vh", overflowY:"auto" }}>
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ display:"block", fontSize:11, color:"#6a5a40", textTransform:"uppercase", letterSpacing:1, marginBottom:6 }}>Muebles</label>
          <div style={{ display:"flex", gap:8 }}>
            <input value={muebleInput} placeholder="Ej: Placard 2 puertas" onChange={e => setMuebleInput(e.target.value)}
            ...
            />
            <button onClick={addMueble} style={{ padding:"11px 18px", background:"#251c0d", border:"1px solid rgba(200,147,58,0.2)", borderRadius:10, color:"#c8933a", fontSize:20, cursor:"pointer" }}>＋</button>
          </div>
          {form.muebles.length > 0 && (
            <div style={{ marginTop:10, display:"flex", flexWrap:"wrap", gap:6 }}>
              {form.muebles.map((m, i) => (
                <div key={i} style={{ background:"#251c0d", border:"1px solid rgba(200,147,58,0.15)", borderRadius:20, padding:"4px 10px 4px 14px", fontSize:13, color:"#e8dcc8", display:"flex", alignItems:"center", gap:6 }}>
                  {m}
                  <button onClick={() => rmMueble(i)} style={{ background:"none", border:"none", color:"#6a5a40", cursor:"pointer", fontSize:12, lineHeight:1 }}>✕</button>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)", backdropFilter:"blur(10px)", zIndex:400, display:"flex", alignItems:"flex-end", justifyContent:"center", padding:"0" }}>
      <div style={{ background:"#13100a", border:"1px solid rgba(200,147,58,0.2)", borderRadius:"24px 24px 0 0", padding:"28px 24px 40px", width:"100%", maxWidth:600, maxHeight:"85vh", overflowY:"auto" }}>
        ...
        <div style={{ fontSize:11, color:"#6a5a40", textTransform:"uppercase", letterSpacing:1, marginBottom:12 }}>
          Lista de muebles
        </div>
        {(obra.muebles||[]).length === 0
          ? <div style={{ color:"#6a5a40", fontSize:13 }}>Sin muebles registrados</div>
          : (obra.muebles||[]).map((m, i) => (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:"1px solid rgba(200,147,58,0.08)" }}>
              <div style={{ fontSize:20 }}>{muebleIcon(m)}</div>
              <div style={{ flex:1, color:"#e8dcc8", fontSize:14 }}>{m}</div>
            </div>
          ))
        }
                ...
                return (
                  <div key={o.id} onClick={() => setDetalle(o)}
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:14 }}>
                      {(o.muebles||[]).slice(0,4).map((m,i) => (
                        <div key={i} style={{ background:"#251c0d", border:"1px solid rgba(200,147,58,0.12)", borderRadius:6, padding:"2px 8px", fontSize:11, color:"#a09070" }}>{m}</div>
                      ))}
